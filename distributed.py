...
def setup_distributed(seed: int = 1337) -> tuple[torch.device, int, int, int, bool]:
    ...
    verify_distributed_setup(world_size, rank, device)
...
def verify_distributed_setup(world_size: int, rank: int, device: torch.device):
    if world_size > 1:
        try:
            ...
            if tensor.item() != world_size:
                raise ValueError(
                    f"All-reduce operation failed. Expected {world_size}, got {tensor.item()}"
                )

            logger.info(f"Rank {rank}: Distributed setup verified successfully.")
        except Exception as e:
            logger.error(f"Rank {rank}: Failed to verify distributed setup: {e}")
            raise
    else:
        logger.info("Standalone setup verified successfully.")
...
def log_system_info():
    ...
    for i in range(torch.cuda.device_count()):
        logger.info(f"GPU {i} name: {torch.cuda.get_device_name(i)}")
...
def load_checkpoint(model_path: str, misc_path: str, model, optimizer, device):
    ...
    misc_checkpoint = torch.load(misc_path, map_location=device)
