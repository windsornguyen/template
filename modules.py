...
class MLP(nn.Module):
    def __init__(self, config, dtype=None):
        ...
        self.h_dim = config.n_embd * config.mlp_scale
        self.gate = nn.Linear(self.n_embd, self.h_dim, bias=config.bias, dtype=dtype)
        self.up_proj = nn.Linear(self.n_embd, self.h_dim, bias=config.bias, dtype=dtype)
        self.down_proj = nn.Linear(self.h_dim, self.n_embd, bias=config.bias, dtype=dtype)
